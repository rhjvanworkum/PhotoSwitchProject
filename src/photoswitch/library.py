...
def generate_smiles(filename, parent_mols, linkers, terminals):
    ...
    for parent_mol in parent_mols:
        ...
        for linker in linkers:
            rxn = AllChem.ReactionFromSmarts(linker_reaction_template + linker)  # type: ignore[attr-defined]
            core = parent_mol
            for _ in range(place_holder_count):
        ...
        for core in unsubstituted_cores:
            place_holder_count = len(core.GetSubstructMatches(substituent_place_holder_mol))
            if place_holder_count == 0:
                all_mols.append(core)
                continue
            for terminal in terminals:
                new_mol = core
                rxn = AllChem.ReactionFromSmarts(terminal_reaction_template + terminal)  # type: ignore[attr-defined]
                for _ in range(place_holder_count):
        ...
        all_mols = [
            Chem.MolFromSmiles(smiles) for smiles in [Chem.MolToSmiles(mol) for mol in all_mols]
        ]
        print(len(all_mols))
